...
export default new Command({
    data: new SlashCommandSubcommandBuilder()
        ...
        .addUserOption((opt) =>
            opt.setName("target").setDescription("User who lose the role")
        )
        .addStringOption((opt) =>
            opt
                .setName("reason")
                .setDescription("Reason to removing the role")
                .setRequired(false)
        ),
    ...
    execute: (interaction: ChatInputCommandInteraction) => {
        ...
        if (!role.editable) {
            embed.setDescription(`I don't have the permissions to manage role ${roleMention(role.id)}`)
        ...
        if (highest.position < role.position) {
            embed
                .setDescription(
                    `You can't remove role ${roleMention(
                        role.id
        ...
        if (!roleManager.cache.has(role.id)) {
            embed
                .setDescription(
                    `
                    ${userMention(member.id)} doesn't have role ${roleMention(
                        role.id
                    )} !
                    `
                )
        ...
        roleManager.remove(role, reason).then((r) => {
            embed.setDescription(
                `Role ${roleMention(role.id)} **removed** to ${userMention(
                    member.id

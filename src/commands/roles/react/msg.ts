...
export default new Command({
    data: new SlashCommandSubcommandBuilder()
        ...
        .addStringOption((opt) =>
            opt
                .setName("name")
                .setDescription("Name of the message reaction")
                .setRequired(true)
        ),
    ...
    execute: async (interaction: ChatInputCommandInteraction) => {
        ...
        console.log("euug", guild);
        if (guild.roleReacts.find((value) => value.name == name)) {
            interaction.reply("Name pas disp");
            return;
        }

        guild.roleReacts.push({ name });
        console.log(guild.roleReacts[0])
        guild.save().then((g) => {
            console.log("CREATED ", g);
            interaction.reply("New message named " + name);
        });
    },
});
/* execute: async (interaction: ChatInputCommandInteraction) => {
